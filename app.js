...
const App = ((() => {
  ...
  cli
    .option('-t, --theme  [name]', `set the vtop theme [${themes}]`)
  ...
  const drawHeader = () => {
    ...
    headerText = ` {bold}MONIT_APP{/bold}{white-fg} for ${os.hostname()} `
    headerTextNoTags = ` MONIT_APP for ${os.hostname()} `
  ...
  const drawChart = chartKey => {
    // console.log('CALLED!!!!', charts, chartKey)
    const chart = charts[chartKey]
    if (!chart) return
    // console.log('chart', chartKey, chart)
    const c = chart.chart
    c.clear()
  ...
  const draw = () => {
    ...
    if (!disableTableUpdate) {
      ...
      const alerts = drawTable(chartKey + 3)
      alertsList.setContent(alerts.title)
      // alertsList.setItems(alerts.body)
      alertsListSelection.setItems(alerts.body)
  ...
  return {
    ...
    init () {
      ...
      alertsListSelection = blessed.list({
        height: alertsList.height - 3,
        top: 1,
        width: alertsList.width - 2,
        left: 0,
        keys: true,
        vi: true,
        search (jump) {
      ...
      alertsList.append(alertsListSelection)
      ...
      const setupCharts = () => {
        ...
        const plugins = ['cpu', 'memory', 'process', 'alerts']
        ...
        for (const plugin in plugins) {
          ...
          switch (plugins[plugin]) {
            ...
            case 'process':
              ...
              break
            case 'alerts':
              width = alertsList.width - 3
              height = alertsList.height - 2
              break  
          }
        ...
        processList.setLabel(` ${charts[2].plugin.title} `)
        alertsList.setLabel(` ${charts[3].plugin.title} `)
      }
      ...
      intervals.push(setInterval(charts[3].plugin.poll, charts[3].plugin.interval))

      // charts.map((x, i) => console.log('ID: ', i, 'name', x))
    }
  }
})())
